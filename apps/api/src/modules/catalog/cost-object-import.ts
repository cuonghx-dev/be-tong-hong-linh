import { CostObjectType } from '@prisma/client'
import * as XLSX from 'xlsx'

export interface ParsedCostObject {
  code: string
  name: string
  type: CostObjectType
...
// Tên cột cần tìm trong header (mẫu Doi_tuong_tap_hop_chi_phi.xlsx).
// Khớp không phân biệt hoa/thường (xem normalize + idx).
const COL = {
  code: ['Mã đối tượng THCP', 'Mã đối tượng', 'Mã'],
  name: ['Tên đối tượng THCP', 'Tên đối tượng', 'Tên'],
  type: ['Loại'],
...
// Cột "Loại": Sản phẩm / Phân xưởng; không khớp (kể cả thiếu cột) → Khác.
function typeFromText(text: string | null): CostObjectType {
  const t = text?.toLowerCase() ?? ''
  if (t.includes('sản phẩm')) return CostObjectType.PRODUCT
  if (t.includes('phân xưởng')) return CostObjectType.WORKSHOP
  return CostObjectType.OTHER
}

// Parse file xlsx đối tượng tập hợp chi phí → danh sách đối tượng.
export function parseCostObjectXlsx(buffer: Buffer): ParsedCostObject[] {
