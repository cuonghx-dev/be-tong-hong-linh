...
export interface ParsedPartnerGroup {
  code: string
  name: string
...
// Tên cột cần tìm trong header (mẫu Danh_sach_nhom_khach_hang_nha_cung_cap.xlsx).
// Khớp không phân biệt hoa/thường (xem normalize + idx).
const COL = {
  code: ['Mã nhóm KH, NCC', 'Mã nhóm', 'Mã'],
  name: ['Tên nhóm khách hàng, nhà cung cấp', 'Tên nhóm', 'Tên'],
...
// Parse file xlsx nhóm KH, NCC → danh sách nhóm.
export function parsePartnerGroupXlsx(buffer: Buffer): ParsedPartnerGroup[] {
