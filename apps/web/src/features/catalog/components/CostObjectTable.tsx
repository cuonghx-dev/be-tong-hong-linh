...
import { CostObjectForm } from './CostObjectForm'
...
// Query params riêng cho bảng đối tượng THCP (tránh đụng param bảng khác cùng trang).
const P = { page: 'dtc_page', q: 'dtc_q', type: 'dtc_type' }

export function CostObjectTable() {
  const [params, setParams] = useSearchParams()
  const [formState, setFormState] = useState<{ costObjectId?: string; readOnly?: boolean } | null>(
    null,
  )
  const del = useDeleteCostObject()
  const importXlsx = useImportCostObjects()
    ...
    importXlsx.mutate(file, {
      onSuccess: (r) =>
        toast({
          ...
          description: `${r.created} đối tượng mới, bỏ qua ${r.skipped} trùng (tổng ${r.total}).`,
        }),
      onError: () =>
        toast({
          variant: 'error',
  ...
  const type = Object.values(CostObjectType).includes(typeParam as CostObjectType)
    ? (typeParam as CostObjectType)
    : undefined

  const filter: CostObjectFilter = {
    page,
    pageSize: PAGE_SIZE,
    keyword: keyword || undefined,
    type,
  }
  const { data, isLoading, isError, refetch, isFetching } = useCostObjects(filter)
    ...
    <Card className="flex h-full flex-col">
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-border p-2">
        ...
        <div className="ml-auto flex items-center gap-2">
          <AddMenu
            actions={[{ label: 'Đối tượng THCP', onClick: () => setFormState({}) }]}
        ...
        <Table className="min-w-[760px]">
          <TableHeader>
            <TableRow>
              ...
              <TableHead>Mã đối&nbsp;tượng THCP</TableHead>
              <TableHead>Tên đối&nbsp;tượng THCP</TableHead>
          ...
          <TableBody>
            ...
            {!isLoading && !isError && rows.length === 0 && (
              <TableRow>
                <TableCell colSpan={7} className="py-10 text-center text-slate-400">
                  Chưa có đối tượng tập hợp chi phí nào.
                </TableCell>
              </TableRow>
            )}
            {rows.map((r) => (
              <TableRow key={r.id} className="group">
                ...
                <TableCell>
                  <button
                    ...
                    onClick={() => setFormState({ costObjectId: r.id, readOnly: true })}
                  >
                    {r.code}
                  </button>
                </TableCell>
                <TableCell className="max-w-[280px] truncate text-slate-700">{r.name}</TableCell>
                <TableCell className="text-slate-600">{COST_OBJECT_TYPE_LABELS[r.type]}</TableCell>
                <TableCell className="max-w-[240px] truncate text-slate-600">{r.description}</TableCell>
                <TableCell>
                ...
                <TableCell className="sticky right-0 z-10 bg-white shadow-[-6px_0_6px_-4px_rgba(0,0,0,0.08)] group-hover:bg-slate-50">
                  <RowActionMenu
                    primaryLabel="Sửa"
                    onPrimary={() => setFormState({ costObjectId: r.id, readOnly: true })}
                    items={[
                      {
                        label: 'Sửa',
                        onClick: () => setFormState({ costObjectId: r.id }),
                      },
                      {
                        label: 'Xóa',
                        danger: true,
                        onClick: async () => {
                          const ok = await confirm({
                            title: `Xóa đối tượng THCP ${r.code}?`,
      ...
      <Modal
        ...
        title={
          formState?.readOnly
            ? 'Xem đối tượng THCP'
            : formState?.costObjectId
              ? 'Sửa đối tượng THCP'
              : 'Thông tin đối tượng THCP'
        }
      >
        {formState && (
          <CostObjectForm
            key={formState.costObjectId ?? 'new'}
            costObjectId={formState.costObjectId ?? null}
