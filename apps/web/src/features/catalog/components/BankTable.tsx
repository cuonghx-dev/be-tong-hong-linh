import type { BankFilter } from '@app/shared'
...
import { useDeleteBank, useImportBanks } from '../api/useBankMutations'
import { BankForm } from './BankForm'

const PAGE_SIZE = 20

// Query params riêng cho bảng ngân hàng (tránh đụng param bảng khác cùng trang).
const P = { page: 'nh_page', q: 'nh_q' }

export function BankTable() {
  const [params, setParams] = useSearchParams()
  const [formState, setFormState] = useState<{ bankId?: string; readOnly?: boolean } | null>(null)
  const del = useDeleteBank()
  const importXlsx = useImportBanks()
    ...
    importXlsx.mutate(file, {
      onSuccess: (r) =>
        toast({
          ...
          description: `${r.created} ngân hàng mới, bỏ qua ${r.skipped} trùng (tổng ${r.total}).`,
        }),
      onError: () =>
        toast({
          variant: 'error',
  ...
  const { data, isLoading, isError, refetch, isFetching } = useBanks(filter)
    ...
    <div className="flex h-full flex-col rounded-lg border border-border bg-white">
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-border p-2">
        ...
        <div className="ml-auto flex items-center gap-2">
          <AddMenu
            actions={[{ label: 'Ngân hàng', onClick: () => setFormState({}) }]}
        ...
        <table className="w-full min-w-[720px] border-collapse text-sm">
          <thead className="sticky top-0 z-20 bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500">
            <tr>
              ...
              <th className="px-3 py-2">Tên viết tắt</th>
              ...
              <tr key={r.id} className="group border-t border-border hover:bg-slate-50">
                ...
                <td className="px-3 py-2">
                  <button
                    ...
                    onClick={() => setFormState({ bankId: r.id, readOnly: true })}
                  >
                    {r.shortName}
                  </button>
                </td>
                <td className="max-w-[420px] truncate px-3 py-2 text-slate-700">{r.fullName}</td>
                <td className="px-3 py-2">
                  <span
                    className={cn(
                ...
                <td className="sticky right-0 z-10 bg-white px-3 py-2 shadow-[-6px_0_6px_-4px_rgba(0,0,0,0.08)] group-hover:bg-slate-50">
                  <RowActionMenu
                    primaryLabel="Sửa"
                    onPrimary={() => setFormState({ bankId: r.id, readOnly: true })}
                    items={[
                      {
                        label: 'Sửa',
                        onClick: () => setFormState({ bankId: r.id }),
                      },
                      {
                        label: 'Xóa',
                        danger: true,
                        onClick: async () => {
                          const ok = await confirm({
                            title: `Xóa ngân hàng ${r.shortName}?`,
      ...
      >
        {formState && (
          <BankForm
            key={formState.bankId ?? 'new'}
            bankId={formState.bankId ?? null}
