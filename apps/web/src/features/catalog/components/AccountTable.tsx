import { ACCOUNT_NATURE_LABELS, type AccountDto, type AccountFilter } from '@app/shared'
...
import { useDeleteAccount, useImportAccounts } from '../api/useAccountMutations'
...
// Query param riêng cho bảng hệ thống tài khoản (tránh đụng param bảng khác cùng trang).
const P = { q: 'httk_q' }
...
// Con mất cha (cha bị lọc/xóa) coi như tài khoản gốc.
function flattenTree(items: AccountDto[], collapsed: Set<string>): TreeRow[] {
...
export function AccountTable() {
  ...
  const importXlsx = useImportAccounts()
  ...
  const { data, isLoading, isError, refetch, isFetching } = useAccounts(filter)

  const items = data?.data ?? []
  const total = data?.pagination.total ?? 0

  // Mặc định thu gọn cây (chỉ hiện TK gốc). Khởi tạo 1 lần khi có dữ liệu.
    ...
    <div className="flex h-full flex-col rounded-lg border border-border bg-white">
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-border p-2">
        ...
        <div className="ml-auto flex items-center gap-2">
          <AddMenu
            actions={[{ label: 'Tài khoản', onClick: () => setFormState({}) }]}
          ...
          <tbody>
            ...
            {rows.map(({ item: r, depth, hasChildren }) => (
              <tr key={r.id} className="group border-t border-border hover:bg-slate-50">
                ...
                <td className="px-3 py-2 text-slate-600">{ACCOUNT_NATURE_LABELS[r.nature]}</td>
                <td className="max-w-[240px] truncate px-3 py-2 text-slate-500">{r.nameEn}</td>
