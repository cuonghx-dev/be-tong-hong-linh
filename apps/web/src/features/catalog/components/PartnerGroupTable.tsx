import type { PartnerGroupFilter } from '@app/shared'
...
import { usePartnerGroups } from '../api/usePartnerGroups'
import { useDeletePartnerGroup, useImportPartnerGroups } from '../api/usePartnerGroupMutations'
...
// Query params riêng cho bảng nhóm KH, NCC (tránh đụng param bảng khác cùng trang).
const P = { page: 'nkn_page', q: 'nkn_q' }

export function PartnerGroupTable() {
  const [params, setParams] = useSearchParams()
  const [formState, setFormState] = useState<{ groupId?: string; readOnly?: boolean } | null>(null)
  const del = useDeletePartnerGroup()
  const importXlsx = useImportPartnerGroups()
  ...
  const filter: PartnerGroupFilter = {
    page,
    pageSize: PAGE_SIZE,
    keyword: keyword || undefined,
  }
  const { data, isLoading, isError, refetch, isFetching } = usePartnerGroups(filter)
    ...
    <Card className="flex h-full flex-col">
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-border p-2">
        ...
        <div className="ml-auto flex items-center gap-2">
          <AddMenu
            actions={[{ label: 'Nhóm KH, NCC', onClick: () => setFormState({}) }]}
        ...
        <Table className="min-w-[720px]">
          <TableHeader>
            <TableRow>
              ...
              <TableHead>Mã nhóm KH, NCC</TableHead>
              <TableHead>Tên nhóm khách&nbsp;hàng, nhà cung&nbsp;cấp</TableHead>
          ...
          <TableBody>
            ...
            {!isLoading && !isError && rows.length === 0 && (
              <TableRow>
                <TableCell colSpan={6} className="py-10 text-center text-slate-400">
                  Chưa có nhóm khách hàng, nhà cung cấp nào.
                </TableCell>
              </TableRow>
            )}
            {rows.map((r) => (
