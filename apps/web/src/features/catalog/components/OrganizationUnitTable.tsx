import { ORG_UNIT_LEVEL_LABELS, type OrganizationUnitDto, type OrganizationUnitFilter } from '@app/shared'
...
// Query param riêng cho bảng cơ cấu tổ chức (tránh đụng param bảng khác cùng trang).
const P = { q: 'cctc_q' }
...
// Con mất cha (cha bị lọc/xóa) coi như đơn vị gốc.
function flattenTree(units: OrganizationUnitDto[], collapsed: Set<string>): TreeRow[] {
  const byId = new Set(units.map((u) => u.id))
  const children = new Map<string | null, OrganizationUnitDto[]>()
  ...
  const walk = (parentId: string | null, depth: number) => {
    for (const unit of children.get(parentId) ?? []) {
      const hasChildren = (children.get(unit.id) ?? []).length > 0
...
export function OrganizationUnitTable() {
  ...
  const [formState, setFormState] = useState<{ unitId?: string; readOnly?: boolean } | null>(null)
  // Mặc định mở rộng toàn bộ cây (cơ cấu tổ chức nhỏ, cần thấy hết phòng ban).
  const [collapsed, setCollapsed] = useState<Set<string>>(new Set())
  const del = useDeleteOrganizationUnit()
  const importXlsx = useImportOrganizationUnits()
    ...
    importXlsx.mutate(file, {
      onSuccess: (r) =>
        toast({
          ...
          description: `${r.created} đơn vị mới, bỏ qua ${r.skipped} trùng (tổng ${r.total}).`,
        }),
      onError: () =>
        toast({
          variant: 'error',
  ...
  const filter: OrganizationUnitFilter = {
    page: 1,
    pageSize: PAGE_SIZE,
    keyword: keyword || undefined,
  }
  const { data, isLoading, isError, refetch, isFetching } = useOrganizationUnits(filter)
  ...
  const rows = useMemo<TreeRow[]>(
    () =>
      treeMode
        ? flattenTree(units, collapsed)
        : units.map((unit) => ({ unit, depth: 0, hasChildren: false })),
    [units, collapsed, treeMode],
  )
    ...
    <div className="flex h-full flex-col rounded-lg border border-border bg-white">
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-border p-2">
        ...
        <div className="ml-auto flex items-center gap-2">
          <AddMenu
            actions={[{ label: 'Đơn vị', onClick: () => setFormState({}) }]}
      ...
      <div className="flex-1 overflow-auto">
        <table className="w-full min-w-[720px] border-collapse text-sm">
          ...
          <tbody>
            ...
            {rows.map(({ unit: r, depth, hasChildren }) => (
              <tr key={r.id} className="group border-t border-border hover:bg-slate-50">
                ...
                <td
                  className={cn(
                    'max-w-[280px] truncate px-3 py-2 text-slate-700',
                    hasChildren && 'font-semibold',
                  )}
                >
                  {r.name}
                </td>
                <td className="max-w-[260px] truncate px-3 py-2 text-slate-600">{r.address}</td>
                <td className="px-3 py-2 text-slate-600">{ORG_UNIT_LEVEL_LABELS[r.level]}</td>
                <td className="px-3 py-2">
                  <span
                    className={cn(
