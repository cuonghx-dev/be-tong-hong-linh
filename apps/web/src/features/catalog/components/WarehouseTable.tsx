import type { WarehouseFilter } from '@app/shared'
...
import { useDeleteWarehouse, useImportWarehouses } from '../api/useWarehouseMutations'
import { WarehouseForm } from './WarehouseForm'
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from '@/shared/ui/table'

const PAGE_SIZE = 20

// Query params riêng cho bảng kho (tránh đụng param bảng khác cùng trang).
const P = { page: 'kho_page', q: 'kho_q' }

export function WarehouseTable() {
  const [params, setParams] = useSearchParams()
  const [formState, setFormState] = useState<{ warehouseId?: string; readOnly?: boolean } | null>(
    null,
  )
  const del = useDeleteWarehouse()
  const importXlsx = useImportWarehouses()
    ...
    importXlsx.mutate(file, {
      onSuccess: (r) =>
        toast({
          ...
          description: `${r.created} kho mới, bỏ qua ${r.skipped} trùng (tổng ${r.total}).`,
        }),
      onError: () =>
        toast({
          variant: 'error',
  ...
  const { data, isLoading, isError, refetch, isFetching } = useWarehouses(filter)
    ...
    <div className="flex h-full flex-col rounded-lg border border-border bg-white">
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-border p-2">
        ...
        <div className="ml-auto flex items-center gap-2">
          <AddMenu
            actions={[{ label: 'Kho', onClick: () => setFormState({}) }]}
              ...
              <TableRow key={r.id} className="group">
                ...
                <TableCell>
                  <button
                    ...
                    onClick={() => setFormState({ warehouseId: r.id, readOnly: true })}
                  >
                    {r.code}
                  </button>
                </TableCell>
                <TableCell className="max-w-[260px] truncate text-slate-700">{r.name}</TableCell>
                ...
                <TableCell className="sticky right-0 z-10 bg-white shadow-[-6px_0_6px_-4px_rgba(0,0,0,0.08)] group-hover:bg-slate-50">
                  <RowActionMenu
                    primaryLabel="Sửa"
                    onPrimary={() => setFormState({ warehouseId: r.id, readOnly: true })}
                    items={[
                      {
                        label: 'Sửa',
                        onClick: () => setFormState({ warehouseId: r.id }),
                      },
                      {
                        label: 'Xóa',
                        danger: true,
      ...
      <Modal
        ...
        size="lg"
        title={
          formState?.readOnly ? 'Xem kho' : formState?.warehouseId ? 'Sửa kho' : 'Thông tin kho'
        }
      >
        {formState && (
          <WarehouseForm
            key={formState.warehouseId ?? 'new'}
