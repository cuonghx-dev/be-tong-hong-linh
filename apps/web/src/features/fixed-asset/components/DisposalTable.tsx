import type { FixedAssetDisposalFilter } from '@app/shared'
...
import { useDeleteDisposal, useImportDisposals } from '../api/useDisposalMutations'
import { useDisposals } from '../api/useDisposals'
import { DisposalFilterPopover, type DisposalFilterValue } from './DisposalFilterPopover'
...
export function DisposalTable() {
  ...
  const del = useDeleteDisposal()
  const importXlsx = useImportDisposals()
  const fileRef = useRef<HTMLInputElement>(null)
  const { toast } = useToast()
  const confirm = useConfirm()

  // Điều hướng sang trang chứng từ full-page (§5).
  const openNew = () => navigate('/fixed-asset/disposals/new')
  const openView = (id: string) => navigate(`/fixed-asset/disposals/${id}`)
  const openEdit = (id: string) => navigate(`/fixed-asset/disposals/${id}/edit`)
  ...
  const filter: FixedAssetDisposalFilter = {
    page,
    pageSize: PAGE_SIZE,
  ...
  const { data, isLoading, isError, refetch, isFetching } = useDisposals(filter)
  ...
  const applyFilter = (v: DisposalFilterValue) => {
    const next = new URLSearchParams(params)
    ...
    <div className="flex h-full flex-col rounded-lg border border-border bg-white">
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-border p-2">
        ...
        <div className="ml-auto flex items-center gap-2">
          <AddMenu
            actions={[{ label: 'Ghi giảm tài sản', onClick: openNew }]}
        ...
        <table className="w-full min-w-[900px] border-collapse text-sm">
          <thead className="sticky top-0 z-10 bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500">
            <tr>
              ...
              <th className="px-3 py-2">Lý do ghi giảm</th>
          ...
          <tbody>
            ...
            {rows.map((r) => (
              <tr key={r.id} className="group border-t border-border hover:bg-slate-50">
                ...
                <td className="max-w-[360px] truncate px-3 py-2 text-slate-700">{r.reason}</td>
                <td className="whitespace-nowrap px-3 py-2 text-right tabular-nums text-slate-700">
                  {formatCurrency(Number(r.totalResidual))}
                </td>
                      ...
                      { label: 'Sửa', onClick: () => openEdit(r.id) },
                      {
                        ...
                        onClick: async () => {
                          const ok = await confirm({
                            ...
                            description: 'Thẻ tài sản liên quan sẽ được khôi phục về "Đang sử dụng".',
                            confirmText: 'Xóa',
                            destructive: true,
                          })
