import { FixedAssetStatus, type FixedAssetFilter } from '@app/shared'
...
import { FIXED_ASSET_STATUS_LABEL } from '../types'
import {
  ...
  return (
    <div className="flex h-full flex-col rounded-lg border border-border bg-white">
        ...
        <table className="w-full min-w-[1900px] border-collapse text-sm">
          <thead className="sticky top-0 z-10 bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500">
            <tr>
              ...
              <th className="px-3 py-2">Loại tài sản</th>
              ...
              <th className="px-3 py-2 text-right">TG còn lại (tháng)</th>
              ...
              <th className="px-3 py-2">Tình trạng sử dụng</th>
          ...
          <tbody>
            ...
            {rows.map((r, i) => (
              <tr key={r.id} className="group border-t border-border hover:bg-slate-50">
                ...
                <td className="whitespace-nowrap px-3 py-2 text-slate-600">{r.assetType}</td>
                ...
                <td className="sticky right-0 z-10 bg-white px-3 py-2 text-right shadow-[-6px_0_6px_-4px_rgba(0,0,0,0.08)] group-hover:bg-slate-50">
                  <button
                    className="font-medium text-red-600 hover:underline"
                    onClick={async () => {
                      const ok = await confirm({
                        title: `Xóa tài sản ${r.code}?`,
          ...
          {!isLoading && !isError && rows.length > 0 && totals && (
            <tfoot className="sticky bottom-0 z-10 bg-slate-100 font-semibold text-slate-700">
              <tr className="border-t-2 border-border">
                ...
                <td className="px-3 py-2 text-right tabular-nums">{totals.remainingMonths}</td>
      ...
      <div className="flex items-center border-t border-border px-3 py-2 text-sm text-slate-500">
        <span>
          Tổng số: <b className="text-slate-700">{total}</b> tài sản
        </span>
...
function StatusBadge({ status }: { status: FixedAssetStatus }) {
  const inUse = status === FixedAssetStatus.InUse
  return (
    <span
      className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${
        inUse ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-500'
      }`}
    >
      {FIXED_ASSET_STATUS_LABEL[status]}
    </span>
  )
}
