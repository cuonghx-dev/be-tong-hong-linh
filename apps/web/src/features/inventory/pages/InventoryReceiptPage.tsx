...
import { useSetReceiptPosted } from '../api/useReceiptMutations'
...
export function InventoryReceiptPage({ mode }: { mode: Mode }) {
  ...
  const { data: voucher } = useReceipt(mode === 'new' ? null : (id ?? null))
  ...
  const quickEdit = () => id && navigate(`/inventory/receipts/${id}/edit`)
  const togglePosted = () => {
