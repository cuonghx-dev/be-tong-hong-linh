import { ProductionOrderStatus, type ProductionOrderFilter } from '@app/shared'
...
import { useProductionOrders } from '../api/useProductionOrders'
import {
...
import {
  ProductionOrderFilterPopover,
  type ProductionOrderFilterValue,
} from './ProductionOrderFilterPopover'
...
const STATUS_CLS: Record<ProductionOrderStatus, string> = {
  [ProductionOrderStatus.NotStarted]: 'bg-slate-100 text-slate-600',
  [ProductionOrderStatus.InProgress]: 'bg-amber-100 text-amber-700',
  [ProductionOrderStatus.Completed]: 'bg-emerald-100 text-emerald-700',
}
...
export function ProductionOrderTable() {
  ...
  const importXlsx = useImportProductionOrders()
  const fileRef = useRef<HTMLInputElement>(null)
  const { toast } = useToast()
  const confirm = useConfirm()

  // Điều hướng sang trang lệnh sản xuất full-page (§5).
  const openNew = () => navigate('/inventory/production-orders/new')
  const openView = (id: string) => navigate(`/inventory/production-orders/${id}`)
  const openEdit = (id: string) => navigate(`/inventory/production-orders/${id}/edit`)
    ...
    importXlsx.mutate(file, {
      onSuccess: (r) =>
        toast({
          ...
          description: `${r.created} lệnh mới, bỏ qua ${r.skipped} trùng (tổng ${r.total}).`,
        }),
      onError: () =>
        toast({
          variant: 'error',
  ...
  const statusParam = (params.get('status') as ProductionOrderStatus | null) ?? null
  const fromDate = params.get('from') ?? ''
  const toDate = params.get('to') ?? ''

  const filter: ProductionOrderFilter = {
    page,
    pageSize: PAGE_SIZE,
  ...
  const { data, isLoading, isError, refetch, isFetching } = useProductionOrders(filter)
  ...
  const applyFilter = (v: ProductionOrderFilterValue) => {
    const next = new URLSearchParams(params)
    for (const [k, val] of Object.entries({ status: v.status, from: v.from, to: v.to })) {
  ...
  return (
    <div className="flex h-full flex-col rounded-lg border border-border bg-white">
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-border p-2">
        <ProductionOrderFilterPopover
          value={{ status: statusParam ?? '', from: fromDate, to: toDate }}
          onApply={applyFilter}
          onReset={resetFilter}
        />
        ...
        <div className="ml-auto flex items-center gap-2">
          <AddMenu
            actions={[{ label: 'Thêm lệnh sản xuất', onClick: openNew }]}
        ...
        <table className="w-full min-w-[1000px] border-collapse text-sm">
          <thead className="sticky top-0 z-10 bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500">
            <tr>
              ...
              <th className="px-3 py-2 text-center">Đã lập đủ PN</th>
              <th className="px-3 py-2 text-center">Đã lập đủ PX</th>
          ...
          <tbody>
            ...
            {rows.map((r) => (
              <tr key={r.id} className="group border-t border-border hover:bg-slate-50">
                ...
                <td className="px-3 py-2">
                  <span
                    className={`inline-block rounded-full px-2 py-0.5 text-xs font-medium ${STATUS_CLS[r.status]}`}
                  >
                    {PRODUCTION_ORDER_STATUS_LABEL[r.status]}
                  </span>
                </td>
