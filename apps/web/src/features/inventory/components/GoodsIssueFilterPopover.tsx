...
export interface GoodsIssueFilterValue {
  category: string
  from: string
  to: string
}

const EMPTY: GoodsIssueFilterValue = { category: '', from: '', to: '' }
...
interface Props {
  value: GoodsIssueFilterValue
  onApply: (v: GoodsIssueFilterValue) => void
  onReset: () => void
}
...
export function GoodsIssueFilterPopover({ value, onApply, onReset }: Props) {
  const [draft, setDraft] = useState<GoodsIssueFilterValue>(value)
  const [preset, setPreset] = useState<Preset>(value.from || value.to ? 'custom' : 'year')

  useEffect(() => setDraft(value), [value])

  const activeCount = (value.category ? 1 : 0) + (value.from || value.to ? 1 : 0)
          ...
          <Field label="Lý do xuất">
            ...
            >
              ...
              <SelectContent>
                ...
                {Object.values(GoodsIssueCategory).map((c) => (
                  <SelectItem key={c} value={c}>
                    {GOODS_ISSUE_CATEGORY_LABEL[c]}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
