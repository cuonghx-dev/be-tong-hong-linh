...
export interface ProductionOrderFilterValue {
  status: string
  from: string
  to: string
}

const EMPTY: ProductionOrderFilterValue = { status: '', from: '', to: '' }
...
interface Props {
  value: ProductionOrderFilterValue
  onApply: (v: ProductionOrderFilterValue) => void
  onReset: () => void
}
...
export function ProductionOrderFilterPopover({ value, onApply, onReset }: Props) {
  const [draft, setDraft] = useState<ProductionOrderFilterValue>(value)
  const [preset, setPreset] = useState<Preset>(value.from || value.to ? 'custom' : 'year')

  useEffect(() => setDraft(value), [value])

  const activeCount = (value.status ? 1 : 0) + (value.from || value.to ? 1 : 0)
