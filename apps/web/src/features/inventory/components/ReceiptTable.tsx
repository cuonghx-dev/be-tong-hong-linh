...
export function ReceiptTable() {
  ...
  const typeParam = (params.get('type') as InventoryReceiptType | null) ?? null
  const fromDate = params.get('from') ?? ''
  const toDate = params.get('to') ?? ''

  const filter: InventoryReceiptFilter = {
    page,
    pageSize: PAGE_SIZE,
    keyword: keyword || undefined,
    receiptType: typeParam ?? undefined,
    fromDate: fromDate || undefined,
    toDate: toDate || undefined,
  }
  const { data, isLoading, isError, refetch, isFetching } = useReceipts(filter)
  ...
  const applyFilter = (v: ReceiptFilterValue) => {
    const next = new URLSearchParams(params)
    for (const [k, val] of Object.entries({ type: v.receiptType, from: v.from, to: v.to })) {
  ...
  const resetFilter = () => {
    const next = new URLSearchParams(params)
    ;['type', 'from', 'to'].forEach((k) => next.delete(k))
    next.set('page', '1')
    setParams(next)
  }
    ...
    <div className="flex h-full flex-col rounded-lg border border-border bg-white">
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-border p-2">
        <ReceiptFilterPopover
          value={{ receiptType: typeParam ?? '', from: fromDate, to: toDate }}
          onApply={applyFilter}
          onReset={resetFilter}
        />
          ...
          <tbody>
            ...
            {rows.map((r) => (
              <tr key={r.id} className="group border-t border-border hover:bg-slate-50">
                ...
                <td className="max-w-[140px] truncate px-3 py-2 text-slate-600">{r.deliverer}</td>
                <td className="px-3 py-2 text-slate-600">{RECEIPT_TYPE_LABEL[r.receiptType]}</td>
