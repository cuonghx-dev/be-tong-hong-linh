...
import { CASH_REPORTS } from '../../types'

// Tab "Báo cáo" phân hệ Tiền mặt: danh sách báo cáo 2 cột (như MISA),
// click 1 báo cáo → trang xem full-page /cash/reports/<slug>.
export function CashReportListTab() {
  const navigate = useNavigate()
  const [keyword, setKeyword] = useState('')

  const reports = CASH_REPORTS.filter((r) =>
      ...
      <div className="rounded-lg border border-border bg-white p-2">
        ...
        <div className="grid grid-cols-1 gap-x-8 lg:grid-cols-2">
          {reports.map((r) => (
            <button
              key={r.slug}
              onClick={() => navigate(`/cash/reports/${r.slug}`)}
