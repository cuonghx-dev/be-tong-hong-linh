...
import { useBankDailyBalance } from '../../api/useBankReports'
import { formatDate, money, StatusRow, tdClass, tdMoney, thClass } from './report-utils'

const COL_SPAN = 5

// Bảng kê số dư tiền theo ngày (tiền gửi) — chỉ liệt kê ngày có phát sinh thu/chi.
export function BankDailyBalanceReport({ filter }: { filter: BankReportFilter }) {
  const { data, isLoading, isError } = useBankDailyBalance(filter)
