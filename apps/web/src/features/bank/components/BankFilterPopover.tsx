...
interface Props {
  ...
  onApply: (v: BankFilterValue) => void
  onReset: () => void
}
...
export function BankFilterPopover({ value, onApply, onReset }: Props) {
  const [draft, setDraft] = useState<BankFilterValue>(value)
            ...
            <Field label="Loại chứng từ">
              <SelectWrap>
                ...
                >
                  ...
                  <option value={BankVoucherType.Receipt}>Thu tiền gửi</option>
                  <option value={BankVoucherType.Payment}>Ủy nhiệm chi</option>
                </select>
              </SelectWrap>
            </Field>
...
// Bọc select + mũi tên (native appearance ẩn).
